...
### На графике справа:
Ось X (Thresholds) отражает уровень порога отсечения — то есть минимальную уверенность модели, при которой прогноз считается валидным и акция попадает в торговую выборку. Диапазон значений (0.5–0.98) показывает, что анализ ведётся для умеренно‑строгих и очень строгих критериев отбора.

Ось Y (Precisions) демонстрирует, как меняется точность прогнозов при повышении порога. На начальном участке Precision стабильно растёт — модель отбирает всё более надёжные сигналы, отсекая неоднозначные случаи. Затем следует резкий всплеск и почти вертикальное падение: это артефакт, связанный с тем, что при очень высоких порогах в выборку попадают лишь единичные прогнозы, и даже один верный сигнал сильно искажает метрику.

Наша задача — найти оптимальный порог в зоне стабильного роста (до всплеска), где модель ещё генерирует достаточное количество сигналов для реализации стратегии, а точность остаётся на приемлемом уровне. Это соответствует точке на графике непосредственно перед началом аномального скачка — именно она будет использоваться как рабочий порог для формирования торговых сигналов.
